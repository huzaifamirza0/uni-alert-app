...
class AdminOfficeChatRoom extends StatefulWidget {
  final String adminOfficeId;
  final String adminOfficeName;

  AdminOfficeChatRoom({
    required this.adminOfficeId,
    required this.adminOfficeName,
  });

  @override
  _AdminOfficeChatRoomState createState() => _AdminOfficeChatRoomState();
}

class _AdminOfficeChatRoomState extends State<AdminOfficeChatRoom> {
  ...
  void onSendMessage(String? imageUrl, String? fileUrl, String? fileName) async {
    if (messageController.text.isNotEmpty || imageUrl != null || fileUrl != null) {
      ...
      await _firestore.collection('adminOffices').doc(widget.adminOfficeId).collection('messages').add(messages);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.adminOfficeName),
      ),
      body: Column(
        children: [
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: _firestore.collection('adminOffices').doc(widget.adminOfficeId).collection('messages')
          ...
          if (userRole == 'student' || userRole == 'hod' || userRole == 'faculty') ...{
            Container(
              ...
              child: SizedBox(
                ...
                child: const Center(child: Text('Only Admin Officer can post messages.',
